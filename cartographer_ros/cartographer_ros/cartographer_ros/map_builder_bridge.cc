...
int MapBuilderBridge::AddTrajectory(
  ...
  std::cout<<"cartographer AddTrejectory(), added trajectory with id : "<<trajectory_id<<std::endl;
...
cartographer_ros_msgs::SubmapList MapBuilderBridge::GetKidnapSubmapList() {
  cartographer_ros_msgs::SubmapList submap_list;
  submap_list.header.stamp = ::ros::Time::now();
  submap_list.header.frame_id = node_options_.map_frame;
  for (const auto& submap_id_pose :
...
MapBuilderBridge::GetTrajectoryStates() {
  ...
  for (const auto& entry : sensor_bridges_) {
    ...
    //LOG(INFO) << "GetTrajectoryStates, trajectory_id is "<<trajectory_id;
