...
Node::Node(
  ...
  submap_kidnaped_list_publisher_=
      node_handle_.advertise<::cartographer_ros_msgs::SubmapList>(
          kSubmapKidnapedListTopic, kLatestOnlyPublisherQueueSize);
  ...
  wall_timers_.push_back(node_handle_.createWallTimer(
      ::ros::WallDuration(node_options_.submap_publish_period_sec),
      &Node::PublishKidnappedSubmapList, this));
...
void Node::PublishKidnappedSubmapList(const ::ros::WallTimerEvent& unused_timer_event) {
  carto::common::MutexLocker lock(&mutex_);
  submap_kidnaped_list_publisher_.publish(map_builder_bridge_.GetKidnapSubmapList());
}
...
bool Node::HandleStartTrajectory(
  ...
  if (!FromRosMessage(request.options, &options) ||
    ...
    LOG(INFO) << "service added trajectory id is " << response.trajectory_id;
...
void Node::HandleLaserScanMessage(const int trajectory_id,
  ...
  carto::common::MutexLocker lock(&mutex_);
  //std::cout<<"Entered node HandleLaserScanMessage" <<std::endll
